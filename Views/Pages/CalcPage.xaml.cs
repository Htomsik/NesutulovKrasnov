...
namespace Praktika.Views.Pages
{
    ...
    public partial class CalcPage : UserControl
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            var number = "";
            var input = (string) ((Button) e.OriginalSource).Content;
            ...
            switch (input)
            {
                ...
                case "%":
                {
                    var s = LabelCalc.Content.ToString();
                    var numbers = Regex.Matches(s, @"(\d+(?:\,\d+)?)")
                        .OfType<Match>()
                        .Select(m =>
                            double.Parse(m.Groups[1].Value, CultureInfo.GetCultureInfo("ru-RU")) *
                            (m.Groups[1].Value.StartsWith("0,0") ? 10 : 1))
                        .ToArray();
                    ...
                    var p = (numbers[0] - numbers[1] * numbers[0]) / 100;
                    LabelCalc.Content = (numbers[0] + str.Replace(",", "") + numbers[1] * numbers[0] / 100);
                    var peer = new ButtonAutomationPeer(Ravno);
